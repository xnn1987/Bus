...
public class VehicleDataBean {
	...
	// 传进来的值
	public VehicleDataBean(float min, float max, String unit) {
		if ("%".equals(unit)) {
			cutedParts = 10;
		} else {
			cutedParts = 8;
		...
		System.out.println(cutedParts+"~~~~~~~~~~~~~~~~~~~`");
		datas = new int[cutedParts + 1];
		this.actualMin = min;
		this.actualMax = max;
		initRange(min, max);
		...
		if (needMin < 0) {
			...
			if (gap <= 100) {
				if (gap < -100) {
					...
					tempMin = tempMax * -1;
					Utils.showTag("max:" + tempMax + "\tmin:" + tempMin);
				}
			...
			tempMax = FormatData(needMax, cutedParts);
			tempMin = 0;
		}
		if (isDealtable) {
