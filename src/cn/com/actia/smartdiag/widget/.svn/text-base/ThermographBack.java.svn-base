...
import cn.com.actia.smartdiag.beans.TempretureBean;
import cn.com.actia.smartdiag.constants.Constants;

public class ThermographBack extends ImageView {
	// private float min;
	// private float max;
	...
	public TempretureBean dataBean;
	public static final Path path = new Path();

	public ThermographBack(Context context, AttributeSet attrs, int defStyle) {
	...
	public ThermographBack(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public ThermographBack(Context context) {
		this(context, null);
	}

	public void initDatas(float min, float max, String unit) {
		dataBean = new TempretureBean(min, max);
		dataBean.unit = unit;
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i <= (markNum) * 2; i++) {
			if (i % 2 == 0) {
				...
				canvas.rotate(240f / (markNum + 1) / 2, centerPoint.x,
						centerPoint.y);
			} else {
		...
		if (null != dataBean.unit && !dataBean.unit.equals("")) {
			canvas.drawText(dataBean.unit, centerPoint.x, 135, labelpaint);
		}

		if (dataBean.ratio != 1) {
			canvas.drawText("X" + dataBean.ratio, centerPoint.x, 155, beipaint);
		}
		...
		// canvas.drawText(
		// "X" + dataBean.getActualMax() + "``" + dataBean.getActualMin(),
		// centerPoint.x, 170, beipaint);
